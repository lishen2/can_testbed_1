...
/*  
Test case 
“44 NM_LimphomeStatus_0301_BeforSILimpHomeSendSARing: Failed ”

1、等待设备启动
2、等待设备休眠发出beep，手工再次启动设备
3、430消息发送opcode为04三次后，等待200ms后发送id为440，长度为8，数据为30 32 00 00 00 00 00 00的消息
*/
...
void TestPoll(void)
{
	if (STATE_WAIT_START == g_state){
		...
		if (3 == g_msg430_04count && TimerGetTick() - g_msg430_04ts > 200){
			SendCanMsg(0x440, 0, "\x30\x32\x00\x00\x00\x00\x00\x00", 8);
